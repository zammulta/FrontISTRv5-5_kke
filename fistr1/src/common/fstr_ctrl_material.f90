...
contains
  ...
  integer function fstr_ctrl_get_PLASTICITY( ctrl, mattype, nlgeom, matval, mattable, dict )
    ...
    if(mattype == GHE) nlgeom = INFINITESIMAL
    if(mattype /= GHE) call setDigit( 1, 1, mattype )
    if(mattype /= GHE) call setDigit( 2, 2, mattype )
    ...
    if(mattype /= GHE) call setDigit( 5, hipt-1, mattype )
    ...
    if(mattype /= GHE) call setDigit( 4, ipt-1, mattype )
  ...
  ! (Gaku Hashimoto, The University of Tokyo, 2014/02/13) <
        !> Read in !JOINT
!####################################################################
        INTEGER FUNCTION fstr_ctrl_get_JOINT( ctrl, mattype, matval )
!####################################################################
...
!--------------------------------------------------------------------
        ...
        INTEGER( KIND = kint ) :: depends, ipt
        
        REAL( KIND = kreal ) :: fval(12)
...
!--------------------------------------------------------------------
        ...
        s = 'GOODMAN,GOODMAN_SEPARATION,DASHPOD_ELEMENT,SLIP_WEAKENING,USER '
        
        IF( fstr_ctrl_get_param_ex( ctrl, 'TYPE ', s, 0, 'P', ipt ) .NE. 0 ) RETURN
        
        ! GOODMAN
        IF( ipt .EQ. 1 ) THEN
         
         fstr_ctrl_get_JOINT                                  &
         = fstr_ctrl_get_data_ex( ctrl, 1, "RRRRRRRRRR ",         &
                                  fval(1), fval(2), fval(3),  &
                                  fval(4), fval(5), fval(6), &
                                  fval(7), fval(8), fval(9), fval(10)) 
         
         IF( fstr_ctrl_get_JOINT .EQ. 0 ) THEN
          
          matval(M_GOODMAN_K1)  = fval(1)
          matval(M_GOODMAN_K2)  = fval(2)
          matval(M_GOODMAN_K3)  = fval(3)
          matval(M_GOODMAN_PHI) = fval(4)
          matval(M_GOODMAN_C)   = fval(5)
          matval(M_GOODMAN_EPS) = fval(6)
          matval(M_DASHPOD_VP)  = fval(7)
          matval(M_PLCONST1)    = fval(8)
          matval(M_PLCONST2)    = fval(9)
          matval(M_PLCONST3)    = fval(10)
        ...
        ELSE IF( ipt .EQ. 2 ) THEN
         ...
         fstr_ctrl_get_JOINT                                 &
         = fstr_ctrl_get_data_ex( ctrl, 1, "RRRR ",          &
                                  fval(1), fval(2), fval(3), &
                                  fval(4), fval(5) )                  
        ...
        ELSE IF( ipt .EQ. 3 ) THEN
         ...
         fstr_ctrl_get_JOINT                                 &
         = fstr_ctrl_get_data_ex( ctrl, 1, "RRR ",          &
                                  fval(1), fval(2), fval(3) )                  
         
         IF( fstr_ctrl_get_JOINT .EQ. 0 ) THEN
          
          matval(M_DASHPOD_VP)  = fval(1)
          matval(M_DASHPOD_VS)  = fval(2)
          matval(M_GOODMAN_C)   = fval(3)
          
          !matval(M_GOODMAN_K1)  = fval(4)  ! not-use 
          !matval(M_GOODMAN_K2)  = fval(5)  ! not-use
          !matval(M_GOODMAN_K3)  = fval(6)  ! not-use
         ...
         mattype = DASHPOD_ELEMENT
        
        ELSE IF( ipt .EQ. 4 ) THEN

         fstr_ctrl_get_JOINT                                  &
         = fstr_ctrl_get_data_ex( ctrl, 1, "RRRRRRRRRRR ",    &
                                  fval(1), fval(2), fval(3),  &
                                  fval(4), fval(5), fval(6),  &
                                  fval(7), fval(8), fval(9),  &
                                  fval(10), fval(11)) 
         ...
         IF( fstr_ctrl_get_JOINT .EQ. 0 ) THEN
          ...
          matval(M_GOODMAN_K2)  = fval(2)
          matval(M_GOODMAN_K3)  = fval(3)
          matval(M_GOODMAN_C)   = fval(4)
          matval(M_PLCONST2)    = fval(5)
          matval(M_PLCONST3)    = fval(6)
          matval(M_DASHPOD_VP)  = fval(7)
          matval(M_PLCONST1)    = fval(8)
          matval(M_DASHPOD_VS)  = fval(9)
          matval(M_GOODMAN_PHI) = fval(10)
          matval(M_GOODMAN_EPS) = fval(11)
         ...
         mattype = SLIP_WEAKENING
         
        ELSE
         
         WRITE(6, *) '!JOINT, TYPE=USER is not supported'
...
!--------------------------------------------------------------------
        ...
        RETURN
!####################################################################
        END FUNCTION fstr_ctrl_get_JOINT
!####################################################################
        ! > (Gaku Hashimoto, The University of Tokyo, 2014/02/13)
        
!####################################################################
        INTEGER FUNCTION fstr_ctrl_get_RO( ctrl, mattype, nlgeom, matval )
!####################################################################
...
!--------------------------------------------------------------------
        ...
        IF( ipt .EQ. 1 ) THEN
         ...
         IF( fstr_ctrl_get_RO .EQ. 0 ) THEN
          
          matval(M_PLCONST1) = fval(1)
          matval(M_PLCONST2) = fval(2)
          matval(M_PLCONST3) = fval(3)
          matval(M_PLCONST4) = fval(4)
        ...
        elseIF( ipt .EQ. 2 ) THEN
         ...
         fstr_ctrl_get_RO                                  &
         = fstr_ctrl_get_data_ex( ctrl, 1, "RRRRRRRR ",         &
                                  fval(1), fval(2), fval(3),  &
                                  fval(4), fval(5), fval(6),  &
                                  fval(7), fval(8)) 
         ...
         IF( fstr_ctrl_get_RO .EQ. 0 ) THEN
          ...
          matval(M_PLCONST3) = fval(3)
          matval(M_PLCONST4) = fval(4)
          matval(M_PLCONST5) = fval(5)
          matval(M_KINEHARD) = fval(6)
          matval(M_GOODMAN_K1) = fval(7)
          matval(M_GOODMAN_K2) = fval(8)
        ...
        ELSE
         ...
         WRITE(6, *) '!Ramberg-Osgood, TYPE=USER is not supported'
...
!####################################################################
        END FUNCTION fstr_ctrl_get_RO
!####################################################################

        !Added by Shanthanu, 2017/03/24
        
!####################################################################
        INTEGER FUNCTION fstr_ctrl_get_concrete( ctrl, mattype, nlgeom, matval )
!####################################################################
...
!--------------------------------------------------------------------
        ...
        s = 'CONCRETE,REBAR,REBAR2D,CONCRETE_EMBED,BOND'
        IF( fstr_ctrl_get_param_ex( ctrl, 'TYPE ', s, 0, 'P', ipt ) .NE. 0 ) RETURN
        ...
        IF( ipt .EQ. 1 ) THEN
            mattype = CONCRETE
            fstr_ctrl_get_concrete                                     &
            = fstr_ctrl_get_data_ex( ctrl, 1, "RRRRRRRRRRRRRRRRRR ",     &
                                  fval(1), fval(2),fval(3),fval(4), &
                                  fval(5), fval(6),fval(7),fval(8), &
                                  fval(9), fval(10),fval(11),fval(12), &
                                  fval(13), fval(14),fval(15),fval(16), &
                                  fval(17),fval(18))
            IF( fstr_ctrl_get_concrete .EQ. 0 ) THEN
			  
              matval(M_YOUNGS)   = fval(1)  !Youngs Modulus
			  matval(M_POISSON)  = fval(2)  !Poisson Ratio
			  matval(5)  = fval(3)          !IROT
			  matval(6)  = fval(4)          !ICIC(1)
			  matval(7)  = fval(5)          !ICIC(2)
			  matval(8)  = fval(6)          !ICIC(3)
			  matval(9)  = fval(7)          !ICIC(4)
			  matval(10) = fval(8)          !ICIC(5)
			  matval(11) = fval(9)          !ICIC(6)
			  matval(12) = fval(10)         !ICIC(7)
			  matval(13) = fval(11)         !ICIC(8)
			  matval(14) = fval(12)         !ICONCYC
			  matval(15) = fval(13)         !FC
			  matval(16) = fval(14)         !ECU
			  matval(17) = fval(15)         !FT
			  matval(18) = fval(16)         !EPCU
			  matval(19) = fval(17)         !EPCUS
			  matval(20) = fval(18)         !C_IC4 !0.6         
        ...
        mattype = REBAR		
         ...
         = fstr_ctrl_get_data_ex( ctrl, 1, "RRR ",         &
				 ...
				 IF( fstr_ctrl_get_concrete .EQ. 0 ) THEN
				  ...
				  matval(M_YOUNGS)   = fval(1)  !Youngs Modulus
				  matval(M_PLCONST1) = fval(2)  !Yield Stress
				  matval(M_PLCONST2) = fval(3)  !LOOP
				 ELSE
        ...
        mattype = REBAR2D		
         ...
         = fstr_ctrl_get_data_ex( ctrl, 1, "RRRR ",         &
				 ...
				 IF( fstr_ctrl_get_concrete .EQ. 0 ) THEN
				  ...
				  matval(M_PLCONST1) = fval(2)  !Yield Stress
				  matval(M_PLCONST2) = fval(3)  !LOOP
				  matval(M_PLCONST3) = fval(4)  !direction of 2-d steel element
				 ELSE
        ...
        mattype = CONCRETE_EMBED
        concrete_embed_flag=1
         fstr_ctrl_get_concrete                                     &
         = fstr_ctrl_get_data_ex( ctrl, 1, "RRRRRRRRRRRRRRRRRR ",     &
                                  fval(1), fval(2),fval(3),fval(4), &
                                  fval(5), fval(6),fval(7),fval(8), &
        ...
        mattype = BOND
         fstr_ctrl_get_concrete                                     &
         = fstr_ctrl_get_data_ex( ctrl, 1, "RRRRRRRRRRRR ",           &
                                  fval(1), fval(2),fval(3),fval(4), &
                                  fval(5), fval(6),fval(7),fval(8), &
                                  fval(9), fval(10),fval(11),fval(12))
				 ...
				 IF( fstr_ctrl_get_concrete .EQ. 0 ) THEN
				  ...
				  matval(M_YOUNGS)   = fval(1)  !BOND ELASTIC MODULUS STEEL DIRECTION
				  matval(5)          = fval(2)  !BOND ELASTIC MODULUS PERPENDICULAR DIRECTION
				  matval(6)          = fval(3)  !STEEL circumferences
				  matval(7)  = fval(4)          !E1BN   -> negative bond elastic modulus
				  matval(8)  = fval(5)          !E2P    -> Positive modulus after bond failure
				  matval(9)  = fval(6)          !E2N    -> Negative modulus after bond failure
				  matval(10) = fval(7)          !TCP    -> ?
				  matval(11) = fval(8)          !TCN    -> ?
				  matval(12) = fval(9)          !TYP    -> ?
				  matval(13) = fval(10)         !TYN    -> ?
				  matval(14) = fval(11)         !ICIC(7)
				  matval(15) = fval(12)         !ICONCYC
				 ELSE
         ...
         WRITE(6, *) '!CONCRETE, TYPE=USER is not supported'
...
!####################################################################
        END FUNCTION fstr_ctrl_get_concrete
!####################################################################
!* ----------------------------------------------------------------------------------------------- *!

...
contains
    ...
    do
      ...
      elseif( header_name == '!CONTACT' ) then
        ...
        ! (Gaku Hashimoto, The University of Tokyo, 2014/02/13) <
          else if( header_name == '!JOINT' ) then
            if( c_material >0 ) then
                if( fstr_ctrl_get_JOINT( ctrl, fstrSOLID%materials(cid)%mtype, fstrSOLID%materials(cid)%variables) /= 0 ) then
                 write(*,*) '### Error: Fail in read in joint definition : ' , cid
                 write(ILOG,*) '### Error: Fail in read in joint definition : ', cid
                 stop
              endif
            endif
          else if( header_name == '!RAMBERG_OSGOOD' ) then
            if( c_material >0 ) then
                if( fstr_ctrl_get_RO( ctrl, fstrSOLID%materials(cid)%mtype, fstrSOLID%materials(cid)%nlgeom_flag, &
                    fstrSOLID%materials(cid)%variables) /= 0 ) then
                 write(*,*) '### Error: Fail in read in RO definition : ' , cid
                 write(ILOG,*) '### Error: Fail in read in RO definition : ', cid
                 stop
              endif
            endif
            
            !Added by Shanthanu, 2017/03/24
          else if( header_name == '!CONCRETE' ) then
            if( c_material >0 ) then
                if( fstr_ctrl_get_concrete( ctrl, fstrSOLID%materials(cid)%mtype, fstrSOLID%materials(cid)%nlgeom_flag, &
                    fstrSOLID%materials(cid)%variables) /= 0 ) then
                 write(*,*) '### Error: Fail in read in concrete definition : ' , cid
                 write(ILOG,*) '### Error: Fail in read in concrete definition : ', cid
                 stop
              endif
            endif  
  ...
  subroutine fstr_solid_alloc( hecMESH, fstrSOLID )
    ...
    ! ADD YM
    allocate ( fstrSOLID%dvnode( ntotal )  ,STAT=ierror )
        if( ierror /= 0 ) then
          write(idbg,*) 'stop due to allocation error <FSTR_SOLID, dvnode>'
          write(idbg,*) '  rank = ', hecMESH%my_rank,'  ierror = ',ierror
          call flush(idbg)
          call hecmw_abort( hecmw_comm_get_comm())
        end if
    allocate ( fstrSOLID%danode( ntotal )  ,STAT=ierror )
        if( ierror /= 0 ) then
          write(idbg,*) 'stop due to allocation error <FSTR_SOLID, danode>'
          write(idbg,*) '  rank = ', hecMESH%my_rank,'  ierror = ',ierror
          call flush(idbg)
          call hecmw_abort( hecmw_comm_get_comm())
        end if
  ...
  subroutine fstr_element_init( hecMESH, fstrSOLID )
    ...
    !--------------------------------------------------------------------
        ...
        ! (Gaku Hashimoto, The University of Tokyo, 2014/02/13) <
        INTEGER( KIND = kint ) :: icel, ic_type, iS, iE, itype
...
!--------------------------------------------------------------------
        ...
        DO itype = 1, hecMESH%n_elem_type
         ...
         IF( ic_type .EQ. 541 ) THEN
          ...
          DO icel = iS, iE
           ...
           IF( ( material%mtype .EQ. GOODMAN ) .OR.              &
               ( material%mtype .EQ. DASHPOD_ELEMENT ) .OR.      &
               ( material%mtype .EQ. SLIP_WEAKENING ) .OR.       &
               ( material%mtype .EQ. GOODMAN_SEPARATION ) ) THEN 
            
            !-----------------------------------------
            
            ndof = getSpaceDimension( fstrSOLID%elements(icel)%etype )
            nn = hecmw_get_max_node( hecMESH%elem_type(icel) )
            
            !-----------------------------------------
            
            ALLOCATE( fstrSOLID%elements(icel)%qf(ndof*nn) )
            
            fstrSOLID%elements(icel)%qf = 0.0D0
  ...
  subroutine fstr_solid_finalize( fstrSOLID )
    ...
    do i=1,size(fstrSOLID%elements)
      ...
      ! (Gaku Hashimoto, The University of Tokyo, 2014/02/13) <
          IF( ASSOCIATED( fstrSOLID%elements(i)%qf ) ) THEN
           
           DEALLOCATE( fstrSOLID%elements(i)%qf )

...
contains
  ...
  subroutine fstr_ass_load(cstep, ctime, hecMESH, hecMAT, fstrSOLID, fstrPARAM,matAssFlag)
    !======================================================================!
    use m_fstr
    use m_static_lib
    use m_fstr_precheck
    ...
    integer, intent(in), optional        :: matAssFlag
    ...
    real(kind=kreal)   :: tt(20), tt0(20), coords(3, 3), factor, factor0
    ...
    do ig0 = 1, fstrSOLID%CLOAD_ngrp_tot
      ...
      do ik = iS0, iE0
        ...
        if(present(matAssFlag)) fstrSOLID%GL(ndof*(in-1)+ityp) = fstrSOLID%GL(ndof*(in-1)+ityp)+(factor - factor0)*fval
      enddo
    enddo
    ...
    do ig0 = 1, fstrSOLID%DLOAD_ngrp_tot
      ...
      do ik = iS0, iE0
        ...
        elseif( ic_type == 241 .or. ic_type == 242 .or. ic_type == 231 .or. ic_type == 232 .or. ic_type == 2322 ) then
          ...
          !added by shan to solve GRAV problem for 2-D plate elements
        vect(1:nn*ndof)=0.0D0
          ...
          call DL_Shell_33(ic_type, nn, ndof, xx, yy, zz, rho, thick, ltype, params, vect, nsize, &
            ...
            else if( ( ic_type==531 ) .or. ( ic_type==541 ) ) then
           ! ????????DLOAD?????????
           vect(1:nn*ndof) = 0.0d0
        ...
        ! ----- Add vector
        do j=1,nsize
          if(present(matAssFlag)) fstrSOLID%GL( iwk(j) )=fstrSOLID%GL( iwk(j) )+(factor - factor0)*vect(j)
        enddo
      enddo
    enddo
    ...
    if( fstrSOLID%TEMP_ngrp_tot > 0 .or. fstrSOLID%TEMP_irres > 0 ) then
      do ig0 = 1, fstrSOLID%TEMP_ngrp_tot
        ...
        do ik = iS0, iE0
          ...
          fstrSOLID%temperature( in ) = pa1+(fval-pa1)*(factor - factor0)
        enddo
      enddo

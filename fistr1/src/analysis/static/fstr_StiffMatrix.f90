...
contains
  ...
  subroutine fstr_StiffMatrix( hecMESH, hecMAT, fstrSOLID, time, tincr, fstrDYNAMIC)
    !---------------------------------------------------------------------*
    use m_static_LIB
    use mMechGauss
    ...
    do itype= 1, hecMESH%n_elem_type
      ...
      if ((fstrSOLID%elements(icel)%gausses(1)%pMaterial%mtype==REBAR2D).and.   &
            ...
            call STF_REBAR2D(ecoord(1:ndof,1:nn),fstrSOLID%elements(icel)%gausses(:),   &
                    thick,stiffness(1:nn*ndof,1:nn*ndof),u(1:2,1:nn))
                    
     !added by shan 2019/05/28
      elseif ((fstrSOLID%elements(icel)%gausses(1)%pMaterial%mtype==BOND).and.   &
            ( ic_type==241)) then
            thick = material%variables(M_THICK)
            call STF_BOND(ecoord(1:ndof,1:nn),fstrSOLID%elements(icel)%gausses(:),   &
        ...
        elseif ( ic_type==301 ) then
          call STF_C1( ic_type,nn,ecoord(:,1:nn),thick,fstrSOLID%elements(icel)%gausses(:),   &
            stiffness(1:nn*ndof,1:nn*ndof), u(1:3,1:nn),du(1:3,1:nn) )
        ...
        elseif (ic_type==341 .or. ic_type==351 .or. ic_type==342 .or. ic_type==352 .or. ic_type==362 ) then
          ...
          call STF_C3                                                                              &
            ...
            call STF_Joint( ic_type,nn,ecoord(:,1:nn),fstrSOLID%elements(icel)%gausses(:), &
                            stiffness(1:nn*ndof,1:nn*ndof), fstrDYNAMIC) 
          ! > (Gaku Hashimoto, The University of Tokyo, 2014/02/13)
          else if ( ic_type == 531 ) then
            call STF_Joint3n( ic_type,nn,ecoord(:,1:nn),fstrSOLID%elements(icel)%gausses(:), &
                            stiffness(1:nn*ndof,1:nn*ndof) ) 

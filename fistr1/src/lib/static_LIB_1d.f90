...
contains
  ...
  subroutine STF_C1( etype,nn,ecoord,area,gausses,stiff, u ,du,temperature )
    use mMechGauss
    ...
    !added by shan
    REAL(kind=kreal),   INTENT(IN), optional :: du(:,:)     !< nodal displacement
    ...
    if ( gausses(1)%pMaterial%mtype== REBAR)elem(:,:)=ecoord(:,:)+u(:,:)+du(:,:)
    ...
    !added by shan
    if ( gausses(1)%pMaterial%mtype== REBAR) then
        if  (gausses(1)%istatus(1)==0)  then
            call calRCSteelInit (outa(1),gausses(1)%istatus,gausses(1)%fstatus, &
                    gausses(1)%pMaterial%variables,llen0)
        else
            outa(1)=gausses(1)%fstatus(1)
        end if
        coeff=outa(1)*area/llen
        stiff(:,:) = 0.d0
        do i=1,3
          do j=1,3
            stiff(i,j) = stiff(i,j) + coeff*direc(i)*direc(j)
          enddo
        enddo
  ...
  !---------------------------------------------------------------------*
  subroutine UPDATE_C1( etype, nn, ecoord, area, u, du, ddu,qf ,iter,gausses, TT, T0 )
    !---------------------------------------------------------------------*
    use m_fstr
    use mMechGauss
    ! I/F VARIABLES
    ...
    !added by Shan for RC Steel
     real(kind=kreal)   :: dstrain
     real(kind=kreal)   :: elembak(3,nn)
     real(kind=kreal)   :: direcbak(3)
     real(kind=kreal)   :: llenbak

    qf(:)              = 0.d0
    ! we suppose the same material type in the element
    elem(:,:) = ecoord(:,:) + u(:,:) + du(:,:)

    !added by shan 2018/02/22
    elembak = ecoord(:,:) + u(:,:)
    direcbak=elembak(:,2)-elembak(:,1)
    llenbak=dsqrt( dot_product(direcbak,direcbak) )
    ...
    if ( gausses(1)%pMaterial%mtype== REBAR) then
        !modified shan 19/01/2018
        if (iter.eq.1) then
        dstrain=(llen-llenbak)/llen0
        gausses(1)%strain(1)=gausses(1)%strain(1)+dstrain
        gausses(1)%fstatus(4) = gausses(1)%strain(1)
        gausses(1)%fstatus(6) = dstrain
        call calRCSteelUpdate(gausses(1)%istatus,gausses(1)%fstatus)
        gausses(1)%stress(1)=gausses(1)%fstatus(5)
        qf(1) = gausses(1)%stress(1)*area
        qf(1:3) = -qf(1)*direc
        qf(4:6) = -qf(1:3)
        gausses(1)%fstatus(23:25)=qf(1:3)
        else
        ! qf(1) = gausses(1)%stress(1)*area
        qf(1:3)=gausses(1)%fstatus(23:25)
        qf(4:6)=-qf(1:3)

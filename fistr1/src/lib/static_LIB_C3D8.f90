...
contains
  ...
  subroutine STF_C3D8Bbar &
    ...
    !mesh dependant parameter calculation added by shan
    if ((gausses(LX)%pMaterial%mtype==CONCRETE.or.gausses(LX)%pMaterial%mtype==CONCRETE_EMBED).and. &
            (gausses(LX)%fstatus(180).eq.0.0D0))gausses(LX)%fstatus(180)=   &
                VOLUME_C3D8(etype, nn, ecoord(1,:), ecoord(2,:), ecoord(3,:))
    ...
    ! Added by Shan 2018/05/14
    ! For Embedded reinforcement
    
    if (gausses(1)%pMaterial%mtype.eq.CONCRETE_EMBED) then
        call calEmbedBarStiff(stiff,gausses(1:8),ecoord)
    end if 
  ...
  subroutine Update_C3D8Bbar                              &
    ...
    !added by shan 
    REAL(kind=kreal), INTENT(IN)      :: ddu(3, nn)    !< displacement increment for concrete
    ...
    real(kind=kreal) :: dstress(6),dstrain(6)
    ...
    if(mtype == RO .or. mtype == GHE) then
        mat_flag=.FALSE.
        totaldisp(:,:) = du(:,:)
    endif
    
    ! For incremental stress/strain 
    if(mtype == CONCRETE.or.mtype==CONCRETE_EMBED) then
        mat_flag=.FALSE.
        totaldisp(:,:) = ddu(:,:)
    endif
    ...
    do LX = 1, NumOfQuadPoints(etype)
      ...
      !YM ADD_ RO????UPDATELAG??????????
          if(mtype == RO .or. mtype == GHE) then
              gausses(LX)%strain(1:6) = gausses(LX)%strain_bak(1:6)+ dstrain(1:6)+EPSTH(:)
              !write(501, '(2I10,100E12.5)') iter, LX, (gausses(LX)%strain(i),i=1,6)
              
              dstress = ( matmul( D(1:6,1:6), dstrain(1:6) ) )
              !write(701, '(2I10,100E12.5)') iter, LX, (gausses(LX)%stress_bak(i),i=1,6), (dstress(i),i=1,6)
              !write(601, '(2I10,100E12.5)') iter, LX, (D(1,i),i=1,3),(D(i,i),i=4,6), (dstrain(i),i=1,6), (dstress(i),i=1,6)
              !write(988, *)  "strain ", dstrain(4:6)
              !write(988, *)  "stress ", dstress(4:6)
              !??????????
              if(iter < 100) call RO_EquivalentStress(gausses(LX), gausses(LX)%stress_bak, dstress, D, iter, &
                                  gausses(LX)%strain_bak, dstrain)
              dstress = ( matmul( D(1:6,1:6), dstrain(1:6) ) )
              !write(601, '(2I10,100E12.5)') iter, LX, (D(1,i),i=1,3),(D(i,i),i=4,6), (dstrain(i),i=1,6), (dstress(i),i=1,6)
              
              gausses(LX)%stress(1) = gausses(LX)%stress_bak(1)+dstress(1)
              gausses(LX)%stress(2) = gausses(LX)%stress_bak(2)+dstress(2)
              gausses(LX)%stress(3) = gausses(LX)%stress_bak(3)+dstress(3)
              gausses(LX)%stress(4) = gausses(LX)%stress_bak(4)+dstress(4)
              gausses(LX)%stress(5) = gausses(LX)%stress_bak(5)+dstress(5)
              gausses(LX)%stress(6) = gausses(LX)%stress_bak(6)+dstress(6)
              ! if((LX == 1) .and. (iter==1)) write(1000 + LX, '(3I10, 100E18.10)') iter, LX, gausses(LX)%istatus(1), &
                                                                                    ! gausses(LX)%strain_bak(1:6), gausses(LX)%stress_bak(1:6), D(1,1), D(4,4)
              !if((LX == 1) .and. (iter==1)) write(2000 + LX,'(I10, 100E12.5)') gausses(LX)%stress_zero_index, &
              !                  gausses(LX)%stress_zero(2, :), gausses(LX)%stress_zero(gausses(LX)%stress_zero_index, :)
              
              gausses(LX)%stress_out(1:6) = gausses(LX)%stress(1:6)
              gausses(LX)%strain_out(1:6) = gausses(LX)%strain(1:6)

        else if(mtype == CONCRETE.or.mtype==CONCRETE_EMBED) then  !Added by Shanthanu 2017/3/24 
                gausses(LX)%istatus(26)= iter               !NIT
                gausses(LX)%fstatus(422)=iter               !CYCN(1,49)
                gausses(LX)%fstatus(471)=iter               !CYCN(2,49)
                gausses(LX)%fstatus(520)=iter               !CYCN(3,49)
                
                ! ! Incremental(modified newton rhapson)
                gausses(LX)%strain=gausses(LX)%strain_bak(1:6)+dstrain(1:6)
                dstress = ( matmul( D(1:6,1:6), dstrain(1:6) ) )
                gausses(LX)%stress(1:6) = gausses(LX)%stress_bak(1:6)+dstress(1:6)
                gausses(LX)%fstatus(1:6) =  gausses(LX)%stress
                gausses(LX)%fstatus(7:12) = gausses(LX)%strain      
                call calUpdateConcrete(D,gausses(LX)%istatus(1:58),gausses(LX)%fstatus(1:560))          
                gausses(LX)%stress(1:6)=gausses(LX)%fstatus(1:6)
                gausses(LX)%stress_bak(1:6)=gausses(LX)%fstatus(1:6)
                gausses(LX)%strain_bak(1:6)=gausses(LX)%fstatus(7:12)

                ! dstrain(1:6) = gausses(LX)%strain_bak(1:6)+ dstrain(1:6)
                ! gausses(LX)%strain(1:6) = dstrain(1:6)
                ! dstrain(1:6) =gausses(LX)%strain(1:6)-gausses(LX)%fstatus(7:12)
                ! dstress = ( matmul( D(1:6,1:6), dstrain(1:6) ) )
                ! gausses(LX)%stress(1:6) = gausses(LX)%fstatus(1:6)+dstress(1:6)
                ! gausses(LX)%fstatus(1:6) =  gausses(LX)%stress
                ! gausses(LX)%fstatus(7:12) = gausses(LX)%strain
                ! call calUpdateConcrete(D,gausses(LX)%istatus,gausses(LX)%fstatus)         
                ! gausses(LX)%stress(1:6)=gausses(LX)%fstatus(1:6)
                
                !Total
                ! gausses(LX)%strain(1:6) = dstrain(1:6)
                ! dstress = (matmul( D(1:6,1:6),dstrain(1:6)-gausses(LX)%strain_bak(1:6)))
                ! gausses(LX)%stress(1:6)   = gausses(LX)%stress_bak(1:6)+dstress(1:6)
                ! gausses(LX)%fstatus(1:6) =  gausses(LX)%stress
                ! gausses(LX)%fstatus(7:12) = gausses(LX)%strain
                ! call calUpdateConcrete(D,gausses(LX)%istatus,gausses(LX)%fstatus)
     ...
     if (gausses(1)%pMaterial%mtype.eq.CONCRETE_EMBED) then
        ! open(34,file='embed_stress',status='unknown')
        totaldisp(:, :) = u(:, :)+du(:, :)
        call calEmbedBarUpdate(qf(1:nn*ndof),u(:, :),totaldisp,gausses(1:8),ecoord,iter)
        ! write(34,*)gausses(1)%fstatus(564),gausses(1)%fstatus(565)
    end if 
  ...
  subroutine TLOAD_C3D8Bbar                    &
   ...
   !> Volume of element
!----------------------------------------------------------------------*
   REAL(kind=kreal) FUNCTION VOLUME_C3D8(etype, nn, XX, YY, ZZ)
!----------------------------------------------------------------------*
...
!---------------------------------------------------------------------
    ...
    DO LX = 1, NumOfQuadPoints(etype)
      ...
      VOLUME_C3D8 = VOLUME_C3D8+getWeight(etype, LX)*det

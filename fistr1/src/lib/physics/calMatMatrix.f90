...
module m_MatMatrix
  ...
  use mUmat
  use m_RO
  use m_Concrete !Added by Shanthanu 2017/3/24
  ...
  subroutine MatlMatrix( gauss, sectType, matrix, time, dtime, cdsys, temperature, isEp, hdflag )
    ...
    elseif( matl%mtype==NORTON ) then
      call iso_creep( matl, sectTYPE, gauss%stress, gauss%strain, gauss%fstatus,  &
        ...
        elseif( matl%mtype==RO .or. matl%mtype==GHE ) then
        call ROMatrix( gauss, gauss%stress,  &
               gauss%istatus(1), gauss%fstatus, matrix, gauss%strain)

!Added by Shanthanu 2017/3/24
      elseif( matl%mtype==CONCRETE ) then
        call calConcreteDmat(gauss%istatus(1:58),gauss%fstatus(1:560),matl%variables,matrix)
        
      elseif( matl%mtype==REBAR2D ) then
        call calRebarDmat(gauss%istatus,gauss%fstatus,matl%variables,matrix)
!Added by Shanthanu 2017/5/14
      elseif( matl%mtype==CONCRETE_EMBED) then
        call calConcreteDmat(gauss%istatus(1:58),gauss%fstatus(1:560),matl%variables,matrix)
!Added by Shanthanu 2019/6/7
      elseif( matl%mtype==BOND) then
        call calBondDmat(gauss%istatus(1:8),gauss%fstatus(1:44),matl%variables,matrix)
    else
